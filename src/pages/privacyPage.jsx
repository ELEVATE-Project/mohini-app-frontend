...
function PrivacyPage() {
  ...
  const [userLanguage, setUserLanguage] = useState(JSON.parse(localStorage.getItem("local_route")) || languageList[0].value);
    ...
    <>
      <div className="container max-w-full md mt-0 mx-auto grid md:grid-cols-2 justify-center">
        <div className="absolute top-6 right-6 min-w-[100px] max-w-fit hidden sm:block">
          <FormData layOut={2} labelName="" id="languageID" selectID="languageID" selectName="language" selectOptions={languageList} labelDivClass="text-left text-slate-700" selectValue={userLanguage} selectClassName="bg-white text-slate-600 rounded-3xl p-3 mt-0 outline outline-slate-300 outline-1 outline-offset min-w-max" selectOnChange={handleLanguageChange} isRequired={true} />
        </div>
        ...
        <div className="">
          <div className="flex justify-between w-full sm:hidden items-center p-2">
            ...
            <div className="w-[140px] flex justify-end p-2">
              <FormData layOut={2} labelName="" id="languageID" selectID="languageID" selectName="language" selectOptions={languageList} labelDivClass="text-left text-slate-700" selectValue={userLanguage} selectClassName="bg-white text-slate-600 rounded-3xl p-3 mt-0 outline outline-slate-300 outline-1 outline-offset min-w-0 w-full" selectOnChange={handleLanguageChange} isRequired={true} />
            </div>
          </div>
            ...
            <>
              <div className="text-center sm:text-2xl mt-[100px] text-md text-slate-700">
                <div className="container max-w-full md mx-auto py-6">
                  <PrivacyPolicyPage tncText={t("tncText")} shouldShowAcceptDecline={false} onAccept={() => {}} onDecline={() => {}} />
                </div>
              </div>
            </>
          </div>
